...
Projeto realizado na Trybe. Primeira aplicação desenvolvida utlizando a arquitetura MSC (Models, Services e Controllers).
...
Foi utilizado o MongoDB para a gestão de dados. Além disso, a API segue o padrão RESTful.

Na aplicação é possível que a pessoa usuária, independente de cadastramento ou login, possa adicionar, ler, deletar e atualizar produtos no seu estoque. O usuário pode também enviar vendas para o sistema. Essas vendas validam se o produto em questão existe. Também é possível ler, deletar e atualizar vendas.

---

- Para **todos os endpoints** eu garanto que:
  ...
  - Todos os retornos seguem o mesmo formato. Para erros que requerem dados adicionais (por exemplo, para informar quais campos estão incorretos) é utlizado a propriedade `data` dentro do objeto `err`.
...
### Cada camada da API está em sua respectiva pasta
...
A conexão do banco local contêm os seguintes parâmetros:
...
O banco possue duas tabelas: produtos e vendas

A tabela de produtos têm o nome: `products`

Os campos da tabela `products` possuem o formato:

```json
{ "name": "Produto Silva", "quantity": 10 }
```

A resposta do insert que retorna após a criação possue o formato:
...
A tabela de vendas têm o nome: `sales`
...
A resposta do insert retornada após a criação possue o formato:
...
Para fazer a análise estática do código, utlizo o lint.
...
Os teste foram feitos pela equipe da Trybe. O [Jest]() e o [Frisby] são usados fazer os testes da api.
...
Abaixo está a lista de requisitos exigidos pela Trybe.
